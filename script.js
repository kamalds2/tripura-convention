...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const galleryDataFull = {
    hall: [
      ...
      { name: 'Guest Arrival', img: 'assets/images/guest-arrival.jpeg' },
      { name: 'Banquet Floor', img: 'assets/images/3d/banquet-floor.jpeg' },
      { name: 'Wedding Stage', img: 'assets/images/wedding-stage1.png' },
...
document.querySelectorAll('[data-show-venue]').forEach(link => {
  ...
  (function initAboutFrameViewer() {
    ...
    const FRAME_PREFIX = 'assets/images/about-frames/frame_000';
  ...
  allForms.forEach(form => {
    ...
    form.addEventListener('submit', async (e) => {
      ...
      const targetEmail = 'KamalKumar.doddi@gmail.com';

      // 1. Format payload data for background API submission
      const payload = {
        access_key: "49538f2e-7172-4714-99cc-c644e6b6ede7", // Public background submission key for KamalKumar.doddi@gmail.com
      ...
      modal.innerHTML = `
        <div class="modal-box success-modal-box" style="max-width:480px; text-align:center; padding:36px 28px; background:#fff; border-radius:16px; box-shadow:0 24px 60px rgba(0,0,0,0.35); position:relative;">
          ...
          <p style="font-size:0.95rem; color:#555; line-height:1.6; margin-bottom:20px;">
            Thank you, <strong id="succName"></strong>! Your details have been sent directly to <strong>Tripura Convention</strong> (<em>KamalKumar.doddi@gmail.com</em>).
          </p>
  ...
  /* ---------- Gallery: Direct click to open full image lightbox modal ---------- */
  // Tapping or clicking any gallery item directly opens the full-screen image modal without an intermediate hover overlay.

  /* ---------- Performance: Smart Video Viewport Controller ---------- */
  // Pauses heavy background videos when off-screen to free CPU/GPU threads and eliminate mobile latency
  if ('IntersectionObserver' in window) {
    ...
    document.querySelectorAll('#aboutVideo, #planVideo, .plan-events-bg-video, .about-anim-canvas').forEach(v => {
  ...
  /* ---------- Aaron Iker Smoke Range Slider Component ---------- */
  (function initSmokeSliders() {
    const random = (min, max) => min + Math.random() * (max - min);
    const between = (min, max, percent) => max - (max - min) * (1 - percent);

    function createSmokeParticle(sliderWrap, input, percent, thumbX) {
      const particle = document.createElement('div');
      particle.classList.add('smoke');
      sliderWrap.appendChild(particle);

      const size = random(between(12, 24, percent), between(24, 38, percent));
      const dur = random(0.7, 1.2);
      const targetY = random(-25, -80);
      const targetX = thumbX + random(-25, 25);
      ...
      function animate(now) {
        ...
        let currY = targetY * progress;
        let currX = thumbX + (targetX - thumbX) * progress;

        particle.style.transform = `translate(${currX - thumbX}px, ${currY}px) scale(${scale})`;
    ...
    document.querySelectorAll('.slider').forEach(sliderWrap => {
      const input = sliderWrap.querySelector('input[type="range"]');
      if (!input) return;

      const valDisplayId = input.id + 'Value';
      ...
      function updateSlider() {
        ...
        const pctFormatted = (percent * 100).toFixed(1) + '%';
        input.style.background = `linear-gradient(90deg, var(--color-gold, #d4af37) 0%, var(--color-gold, #d4af37) ${pctFormatted}, rgba(200, 200, 200, 0.35) ${pctFormatted}, rgba(200, 200, 200, 0.35) 100%)`;
      ...
      function spawnSmoke() {
        ...
        const val = parseFloat(input.value) || min;
        const percent = (val - min) / (max - min);
        const thumbX = percent * (rect.width - 24);

        createSmokeParticle(sliderWrap, input, percent, thumbX);
      }
      ...
      input.addEventListener('pointerdown', () => {
        ...
        smokeInterval = setInterval(spawnSmoke, 80);
      });
